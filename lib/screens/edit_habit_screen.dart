...
class EditHabitScreen extends StatefulWidget {
  final Habit habitToEdit; // The habit object passed from HabitSettingScreen

  const EditHabitScreen({super.key, required this.habitToEdit});
...
class _EditHabitScreenState extends State<EditHabitScreen> {
  ...
  void initState() {
    ...
    // _reminderTimeController.text initialization depends on context, move it to didChangeDependencies
    _reminderTimeController = TextEditingController(); // Initialize here without context
  }
  ...
  Future<void> _selectDate(BuildContext context, {required bool isStartDate}) async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate: DateTime(2020, 1, 1), // Allow selecting past dates for existing habits
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (user == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('You must be logged in to update a habit.')),
        );
        if (!mounted) return;
        setState(() {
          _isLoading = false;
      ...
      try {
        ...
        print('DEBUG: Updated Habit object:');
        print('   Name: ${updatedHabit.name}');
        print('   User ID: ${updatedHabit.userId}');
        print('   ID: ${updatedHabit.id}'); // Ensure ID is present for update

        await _habitService.updateHabit(updatedHabit);

        print('DEBUG: Habit updated successfully in Firestore.');

        // --- Notification Rescheduling Logic ---
        // Cancel existing notifications for this habit
        await _notificationService.cancelNotificationsForHabit(updatedHabit.id!);
        print('DEBUG: Canceled existing notifications for habit ID: ${updatedHabit.id}');

        // Schedule new notifications if reminder time is set and permission granted
        if (status.isGranted && _selectedReminderTime != null) {
          for (int i = 0; i < 7; i++) {
        ...
        if (mounted) {
          ...
          Navigator.pop(context); // Go back to HabitSettingScreen
        }
        print('DEBUG: _updateHabit process finished.');
      } on FirebaseException catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error updating habit: ${e.message}')),
          );
        }
                ...
                _isLoading
                    ...
                    : SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _updateHabit, // Call _updateHabit
  ...
  // Re-used helper widgets from CreateNewHabitScreen
